// Página Social: contas conectadas e fluxo de conexão
import './styles.css'

// Página que mostra o estado das contas sociais conectadas.
function Social() {
  const accounts = [
    { platform: 'YouTube', username: '@streamify', status: 'Conectada', color: 'online' },
    { platform: 'Twitch', username: '@streamify_live', status: 'Pendente', color: 'offline' },
    { platform: 'LinkedIn', username: 'Streamify Media', status: 'Conectada', color: 'online' },
  ]
          ...
          <ul className="list-stack">
            {accounts.map((account) => (
              <li key={account.platform} className="list-item">
                ...
                <span className={`status-pill ${account.color}`}>{account.status}</span>
              </li>
            ))}
          </ul>
        </article>
