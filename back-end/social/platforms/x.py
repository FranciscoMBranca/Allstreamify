...
def normalize_x_payload(payload: dict[str, Any]) -> dict[str, Any]:
    """Normaliza os dados recebidos para uma conta do X (Twitter)."""
    normalized = {
        'platform_user_id': _pick_value(payload, 'platformUserId', 'platform_user_id', 'userId', 'id'),
        'username': _pick_value(payload, 'username', 'screenName', 'handle') or 'x_user',
        'display_name': _pick_value(payload, 'displayName', 'name', 'username', 'screenName') or '',
    ...
    if not normalized['platform_user_id']:
        raise ValueError('X payload deve conter platformUserId, platform_user_id, userId ou id.')
